...
const DateForm = ({ data, updateFieldHandler }) => {
  ...
  function formatarDataParaBanco(data: number | Date) {
    return format(data, "dd/MM/yyyy"); // Formate a data para "dd/mm/aaaa"
  }

  function formatarDataParaInput(data: number | Date) {
    ...
    <div>
      <div className="date">
        ...
        <input
          ...
          id="calendario"
          max={dataMaxima}
          onChange={(e) => updateFieldHandler("date", formatarDataParaBanco(new Date(e.target.value)))}
          required
        ></input>
      </div>
    </div>
